...
public interface AnalyticsRepository extends JpaRepository<Order, Long> {
    ...
    @Query("""
                ...
                ORDER BY COUNT(o) DESC
            """)
    List<Object[]> topCustomersByFrequency(@Param("from") Instant from, @Param("to") Instant to);
    ...
    @Query(value = """
                SELECT TO_CHAR(created_at AT TIME ZONE 'UTC', 'Day') AS day_name,
                       COUNT(*) AS order_count,
                       SUM(total) AS total_sales
                FROM orders
                WHERE status = 'PAID' AND created_at BETWEEN :from AND :to
                GROUP BY TO_CHAR(created_at AT TIME ZONE 'UTC', 'Day'),
