...
public class AnalyticsService {
    ...
    public AnalyticsSummaryByDTO getSummary(LocalDate from, LocalDate to, int limit) {
        ...
        for (Object[] r : repo.takeOutVsDineIn(start, end)) {
            ...
            if (isTakeOut) {
                takeOutCount = toLong(r[1]);
                takeOutRevenue = toDouble(r[2]);
            } else {
                dineInCount = toLong(r[1]);
                dineInRevenue = toDouble(r[2]);
            }
        }

        return AnalyticsSummaryByDTO.builder()
                // KPIs
                ...
                .topRoomsBySales(cap(mapRoom(repo.topRoomsBySales(start, end)), limit))
                ...
                .topRoomsByCount(cap(mapRoom(repo.topRoomsByCount(start, end)), limit))
                ...
                .leastRoomsBySales(cap(mapRoom(repo.leastRoomsBySales(start, end)), limit))
                ...
                .leastRoomsByCount(cap(mapRoom(repo.leastRoomsByCount(start, end)), limit))
    ...
    private List<RoomSalesDTO> mapRoom(List<Object[]> rows) {
        return rows.stream()
                .map(r -> new RoomSalesDTO(toLong(r[0]), (String) r[1], toDouble(r[2]), toLong(r[3])))
                .toList();
    }
    ...
    private List<MenuItemSalesDTO> mapMenuItem(List<Object[]> rows) {
        return rows.stream()
                .map(r -> new MenuItemSalesDTO(toLong(r[0]), (String) r[1], (String) r[2], toDouble(r[3]),
                        toLong(r[4])))
                .toList();
    }
    ...
    private List<CustomerSalesDTO> mapCustomer(List<Object[]> rows) {
        return rows.stream()
                .map(r -> new CustomerSalesDTO(toLong(r[0]), (String) r[1], (String) r[2], toDouble(r[3]),
                        toLong(r[4])))
                .toList();
    }
    ...
    private long nullSafeLong(Long val) {
