...
public class TopMenuItemService {
    ...
    private final TopMenuItemRepository topMenuItemRepository;
    private final ZoneId zoneId = ZoneId.systemDefault();

    public TopMenuItemResponse getTopMenuItems(DashboardPeriod period, int limit) {
        // 1. Establish explicit, zone-aware boundaries to match split frameworks
        ...
        // Enforce hard cap limit to a maximum of 7 items for dashboard aesthetics
        ...
        List<MenuItemRevenueData> results = topMenuItemRepository.findTopMenuItemsByRevenue(
                OrderStatus.PAID,
                start,
                end,
                pageable
        ...
        if (results.isEmpty()) {
            return new TopMenuItemResponse(items);
        }

        // 3. Find max revenue safely using the new Double data type mapping structure
        ...
        // 4. Transform native record maps into clean payload items
        int rank = 1;
        for (MenuItemRevenueData data : results) {
            BigDecimal revenue = data.totalRevenue() != null ? BigDecimal.valueOf(data.totalRevenue()) : BigDecimal.ZERO;
            
            // Calculate percentage share relative to the top performing menu item
            int percent = revenue
            ...
            items.add(new Item(
                    rank++,
                    data.menuItemName() != null ? data.menuItemName() : "Unknown Item",
